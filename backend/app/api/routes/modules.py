...
async def read_module_route(
    session: SessionDep, current_user: CurrentUser, module_id: str
):
...
async def update_module_route(
    ...
    module_data = module_in.model_dump(
        exclude_unset=True, exclude={"attachments", "quizzes"}
    )
    ...
    if "attachments" in module_in.model_fields_set:  # explicitly passed in request
        attachments = await _get_objects_by_id(
            Attachment, module_in.attachments, session
        )
        for index, attachment in enumerate(attachments):
            attachment.order = index + 1
...
async def reorder_modules(
    session: SessionDep, course_id: str, order_items: list[ModuleOrderItem]
):
    # 1. Fetch all modules in course
    result = await session.exec(select(Module).where(Module.course_id == course_id))
    ...
    if set(course_modules.keys()) != set(received_ids):
        raise HTTPException(
            400, "All modules in the course must be included in reorder request"
        )
